    ...
    public function compose(View $view)
    {
        ...
        if (auth()->check()) {
            ...
            \Illuminate\Support\Facades\Log::info('Current User:', [
                ...
                // Removed getAllPermissions() call that was causing an error
            ]);
...
ORDER BY 
    m1.`order`, m2.`order`;
        ...
        //     AND m1.is_active = 1
        //     AND m1.is_visible = 1
        //     AND (m2.is_active IS NULL OR m2.is_active = 1)
        //     AND (m2.is_visible IS NULL OR m2.is_visible = 1)
        // ORDER BY 
        //     m1.order, m2.order";  $query = 'SELECT 
        //     m1.id, 
        //     m1.name, 
        //     m1.slug, 
        //     m1.icon, 
        ...
        foreach ($results as $row) {
            ...
            $parentItem = [
                ...
                'name' => $row->name,
                'url' => $row->url,
                'path' => $this->getMenuItemPathFromData($row->route_name, $row->url),
                'icon' => $row->icon,
                'order' => $row->order,
                'children' => []
            ];
